...
session_start();
	 ...
	 while($loop_count!=0)
	{
		$statement = $this->dbConnection->prepare('SELECT video_id,count,video_url FROM video_details where count < 6 ORDER BY RAND() LIMIT 1');
		if($statement->execute())
       {
		   	  foreach ($statement as $rows)
			{
...
session_start();
 ...
 if($i%2==0)		
	...
	echo "<td><img src='".$image_path."' height='200' width='250'></td><td><a href='annotation.php?id=".$video_id." '>".$video_name."</a></td>";		
if($i%2==1){		
		echo "</tr><br>";		
	}		
		$i=$i+1;	 		
